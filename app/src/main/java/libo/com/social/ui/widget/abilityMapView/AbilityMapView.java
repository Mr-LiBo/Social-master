package libo.com.social.ui.widget.abilityMapView;
...
public class AbilityMapView extends View {
    ...
    private void initSize(Context context) {
        n = 7;  //七条边
        R = dp2pxF(context, 150);  //半径暂时设为100dp
        intervalCount = 4;   //有四层
...
//        canvas.drawLine(0, -(viewWidth / 2), 0, viewWidth / 2, linePaint);
     ...
     * @param canvas
     */
    private void drawPolygone(Canvas canvas) {
        //保存画布当前状态
        canvas.save();
        //设置为填充且描边
    ...
    private void drawAbilityLine(Canvas canvas) {
        ...
        for (int i = 0; i < n; i++) {
            //能力值 /100 再乘以半径就是所点的比例
            float r = R * (allAbility[i] / 100.0f);
    ...
    private void drawAbilityText(Canvas canvas) {
        ...
        for (int i =0;i< n;i++){
            float r = R+ dp2px(getContext(),15f);
            float x = (float) (r * Math.cos(i*angle - Math.PI/2));
            float y = (float) (r * Math.sin(i*angle - Math.PI/2));
